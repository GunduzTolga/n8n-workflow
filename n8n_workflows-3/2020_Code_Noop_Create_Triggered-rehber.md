...
Bu rehber `2020_Code_Noop_Create_Triggered.json` dosyasındaki iş akışının ayrıntılı kurulumu ve kullanımı için hazırlanmıştır.
...
## Kurulum Adımları
1. n8n arayüzünde **"Workflow" → "Import from file"** seçeneği ile `2020_Code_Noop_Create_Triggered.json` dosyasını içe aktarın.
...
## Çalıştırma
İş akışı tetiklendiğinde ilgili adımlar çalışarak addon for workflow nodes update check template işlemini otomatikleştirir. Manuel olarak "Execute Workflow" butonu ile ya da tetikleyici node'un koşullarına göre çalıştırılabilir.
