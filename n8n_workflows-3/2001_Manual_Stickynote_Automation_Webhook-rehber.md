...
Bu rehber `2001_Manual_Stickynote_Automation_Webhook.json` dosyasındaki iş akışının ayrıntılı kurulumu ve kullanımı için hazırlanmıştır.
...
## Kurulum Adımları
1. n8n arayüzünde **"Workflow" → "Import from file"** seçeneği ile `2001_Manual_Stickynote_Automation_Webhook.json` dosyasını içe aktarın.
...
## Çalıştırma
İş akışı tetiklendiğinde ilgili adımlar çalışarak brand content extract, summarize & sentiment analysis with bright data işlemini otomatikleştirir. Manuel olarak "Execute Workflow" butonu ile ya da tetikleyici node'un koşullarına göre çalıştırılabilir.
