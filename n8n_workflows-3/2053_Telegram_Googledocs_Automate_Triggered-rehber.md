...
Bu rehber `2053_Telegram_Googledocs_Automate_Triggered.json` dosyasındaki iş akışının ayrıntılı kurulumu ve kullanımı için hazırlanmıştır.
...
## Kurulum Adımları
1. n8n arayüzünde **"Workflow" → "Import from file"** seçeneği ile `2053_Telegram_Googledocs_Automate_Triggered.json` dosyasını içe aktarın.
...
## Çalıştırma
İş akışı tetiklendiğinde ilgili adımlar çalışarak 🤖🧠 ai agent chatbot + long term memory + note storage + telegram işlemini otomatikleştirir. Manuel olarak "Execute Workflow" butonu ile ya da tetikleyici node'un koşullarına göre çalıştırılabilir.
