...
Bu rehber `2025_Splitout_Code_Automate_Webhook.json` dosyasındaki iş akışının ayrıntılı kurulumu ve kullanımı için hazırlanmıştır.
...
## Kurulum Adımları
1. n8n arayüzünde **"Workflow" → "Import from file"** seçeneği ile `2025_Splitout_Code_Automate_Webhook.json` dosyasını içe aktarın.
...
## Çalıştırma
İş akışı tetiklendiğinde ilgili adımlar çalışarak clone n8n workflows between instances using n8n api işlemini otomatikleştirir. Manuel olarak "Execute Workflow" butonu ile ya da tetikleyici node'un koşullarına göre çalıştırılabilir.
